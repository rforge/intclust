ClusterDistribution <- function(List,Selection,nrclusters,followMaxComps=FALSE,followClust=TRUE,fusionsLog=TRUE,WeightClust=TRUE,names=NULL,reverse=FALSE,Plot=TRUE,Table=TRUE,CompletePlot=FALSE,cols){
	...
	ClusterDistribution.2<-function(List,Selection,nrclusters,followMaxComps,followClust,fusionsLog,WeightClust,reverse){
		if ((length(List)==1 & (class(List[[1]])=="Weighted"| class(List[[1]])=="CEC"| class(List[[1]])=="WeightedSim")) & reverse==TRUE){
			...
			if((class(List[[2]])=="Weighted"| class(List[[2]])=="CEC"|class(List[[2]])=="WeightedSim") & reverse==TRUE){
		...
		label.interest=unique(Matrix[1,which(colnames(Matrix)%in%Selection)])
		...
		for(i in 1:dim(Matrix)[1]){
			...
			if(followClust==TRUE){
				...
				complabels=names(Matrix[i,which(colnames(Matrix)%in%Selection & Matrix[i,]==as.numeric(label.interest))])
				temp[[7]]=complabels
				names(temp)[7]="Complabels"
				complete.new.cluster=names(Matrix[i,which(Matrix[i,]==as.numeric(label.interest))])
				temp[[8]]=complete.new.cluster
				names(temp)[8]="Complete.new.cluster"
				extra.new.cluster=complete.new.cluster[which(!(complete.new.cluster %in% Selection))]
				temp[[9]]=extra.new.cluster
	...
	Found=ClusterDistribution.2(List,Selection,nrclusters,followMaxComps,followClust,fusionsLog,WeightClust,reverse)
	...
	if(Plot==TRUE){
		...
		#	OneLess[[i]]=ClusterDistributionSep(List[[i]],Selection,K-1,Height,extraObjects,OriginalExtra,MaxExtra)		
		#}
		
		#OneMore=list()
		#for(i in 1:length(List)){
		#	OneMore[[i]]=ClusterDistributionSep(List[[i]],Selection,K+1,Height,extraObjects,OriginalExtra,MaxExtra)		
		#}
		...
		#oneless=c()
		#for(j in 1:length(OneLess)){
		#	oneless=c(oneless,OneLess[[j]]$nr.clusters)
		#}
		
		#onemore=c()
		#for(j in 1:length(OneMore)){
		#	onemore=c(onemore,OneMore[[j]]$nr.clusters)
		#}
		
		plot(type="n",x=0,y=0,xlim=c(1,length(Found)),ylim=c(0,max(nrcluster,nrcomps)+2.0),xlab="",ylab="",xaxt="n",yaxt="n",cex.lab=1.25)
		...
		#lines(x=seq(1,length(Found)),y=oneless,lty=1,col="grey")
		#lines(x=seq(1,length(Found)),y=onemore,lty=1,col="grey")
		
		axis(1,at=seq(1,length(Found)),labels=names,las=2,cex=1)
		axis(2,at=seq(0,max(nrcluster,nrcomps)+2.0),labels=seq(0,max(nrcluster,nrcomps)+2.0),cex=1)
		
		if(followMaxComps==TRUE){
			legend(1,max(nrcluster,nrcomps)+2.7,lty=c(1,1,0),pch=c(19,19,0),col=c("blue","red","black"),legend=c(lab1,"Number of clusters original cluster divided amongst","Cluster number"),bty="n",cex=1.2)
		}
		else{legend(1,max(nrcluster,nrcomps)+2.7,lty=c(1,1),pch=c(19,19),col=c("blue","red"),legend=c(lab1,"Number of clusters original cluster divided amongst"),bty="n",cex=1.2)}
	}
	...
	if(CompletePlot==TRUE){
		...
		plot(type="n",x=0,y=0,xlim=c(1,length(Found)),ylim=c(0,max(nrcluster,nrcomps)+0.5),xlab="",ylab="Number of Compounds",xaxt="n",yaxt="n")
		...
		for(m in 1:length(Found)){
			...
			#xpoints=rep(seq(1,length(Found))[m],howmany)
			#ypoints=c()
			if(m==1){
				...
				for(g in 1:howmany){				
					...
					points(x=xnext[g],y=ynext[length(ynext)],col=colorsp[length(colorsp)],pch=19,cex=1.25)
					labelcluster=Found[[m]]$AllClusters[[g]]$clusternumber
					position=3
					if(!(is.integer(ynext[length(ynext)]))){
						position=1
					}
					text(xnext[g],ynext[length(ynext)],labelcluster,cex=1.5,pos=position,col="black",font=2)					
				}
				L1=list()
				for(n in 1:howmany){
				...
				#text(xnext[n],ynext[length(ynext)],labelcluster,cex=1.0,pos=3,col="black",font=2)			
		...
		axis(1,at=seq(1,length(Found)),labels=names,las=2,cex=1.5)
		axis(2,at=seq(0,max(nrcluster,nrcomps)),labels=seq(0,max(nrcluster,nrcomps)),cex=1.5)
		legend(1,max(nrcluster,nrcomps)+1,pch=c(0),col=c("black"),legend=c("Cluster number"),bty="n",cex=1.2)
