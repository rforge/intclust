SharedSelection<-function(DataLimma=NULL,DataMLP=NULL,names=NULL){  #Input=result of DiffGenes.2 and Geneset.intersect
	
	if(is.null(DataMLP)){
		ResultShared=SharedSelectionLimma(DataLimma,names)		
	}
	else if(is.null(DataLimma)){
		ResultShared=SharedSelectionMLP(DataMLP,names)		
	}
