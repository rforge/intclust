PathwaysIter <-
function(List,GeneExpr=geneMat,nrclusters=7,method=c("limma", "MLP"),ENTREZID=GeneInfo[,1],geneSetSource = "GOBP",top=NULL,GENESET=GS,sign=0.05,niter=10,fusionsLog=TRUE,WeightClust=TRUE,names=NULL){
	ListNew=list()
	element=0
	...
	if(length(List)==1){
		...
		for (i in 1:niter){
			print(paste("Iteration",i,sep=" "))
			comps.limma.mlp = Pathways(List,GeneExpr,nrclusters,method,ENTREZID,geneSetSource,top,GENESET,sign=0.05,fusionsLog,WeightClust,names)
			List.output[[length(List.output)+1]] = comps.limma.mlp
			names(List.output)[i]=paste("Iteration",i,sep=" ")
			#List.cutpoints[[length(list.cutpoints)+1]] = sign		 
		}
